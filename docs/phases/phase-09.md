...
- **Google Sheets**: import contacts from the first tab when it follows the exact `name`, `phone` column contract; write back after every terminal call and on demand to two tabs, `Tokito answers` and `Tokito calls`, using the Phase 6 export rows so the sheet and the CSV export never differ.
