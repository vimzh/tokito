...
export type StructuredRun = <T extends z.ZodType>(args: { system: string; prompt: string; schema: T }) => Promise<{ output: z.output<T>; model: string }>
...
export function createStructuredRun(model = createOpenAiModel()): StructuredRun {
  return async ({ system, prompt, schema }) => {
    const agent = new Agent({ model, systemPrompt: system, structuredOutputSchema: schema, printer: false })
    let result
    try {
      ...
      if (error instanceof StructuredOutputError) throw new AiRequestError('The model did not return a usable questionnaire.')
      if (error instanceof ModelError) throw new AiRequestError(`OpenAI request failed: ${error.message}`)
      throw error
    }
    if (result.structuredOutput === undefined) throw new AiRequestError('The model did not return a usable questionnaire.')
