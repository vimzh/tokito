...
import { updateContactSchema } from '../validation/contacts'
...
export const contactRoutes = new Hono()
  ...
  .post('/:id/contacts/imports', zValidator('form', z.object({ file: z.instanceof(File) }), validationHook), async (c) => {
    ...
    const preview = service.createImport(db, c.req.param('id'), file.name, await file.arrayBuffer())
    return c.json(service.commitImport(db, c.req.param('id'), preview.id), 201)
  })
